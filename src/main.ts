...
const selectDifficulty = (content: HTMLElement) => {
  [
    { lbl: "easy", size: 10, mines: 10 },
    { lbl: "medium", size: 16, mines: 40 },
    { lbl: "hard", size: 22, mines: 99 },
  ].forEach((dif, i) => {
...
const startGame = (size: number, mines: number, content: HTMLElement) =>
  new Game(content, size, mines, () => {
    const e = document.createElement("div");
